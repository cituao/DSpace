...
<!--<h2><fmt:message key="jsp.search.title"/></h2>título del SEARCH-->

<div class="discovery-search-form panel panel-default" id="filtersContainer"> <!-- id filterContainer personalizado -->
...
<% if (appliedFilters.size() > 0 ) { %>                                
		...
		<%
			...
			for (String[] filter : appliedFilters)
			{
			    boolean found = false;
			    %>
                <div class="filterTag"> <!-- container for tags -->
				...
				<input class="btn btn-default" type="submit" id="submit_filter_remove_<%=idx %>" name="submit_filter_remove_<%=idx %>" value="" />
				<br/>
                    </div>
				<%
				idx++;
...
<% } %>
<a class="btn btn-default" id="newSearch" href="<%= request.getContextPath()+"/simple-search" %>"><!--<fmt:message key="jsp.search.general.new-search" />--><img src="<%= request.getContextPath() %>/image/close.svg"  onerror="this.onerror=null; this.src='<%= request.getContextPath() %>/image/close.png'" alt="DSpace logo" id="newSearchButton"/></a>	
		</form>
                    </div>
		</div>
<% if (availableFilters.size() > 0) { %>
    <div class="container">
		...
		<input type="text" id="filterquery" placeholder="  'Don Quijote'" name="filterquery" size="45" required="required" />
		<input type="hidden" value="<%= rpp %>" name="rpp" />
		<input type="hidden" value="<%= sortedBy %>" name="sort_by" />
		<input type="hidden" value="<%= order %>" name="order" />
		<input class="btn btn-primary" type="submit" value="<fmt:message key="jsp.search.filter.add"/>" onclick="return validateFilters()" />
		</form>
		</div>        
<% } %>
...
<%
           ...
           {
%>
               <label for="sort_by"><!--<fmt:message key="search.results.sort-by"/> DESCOMENTAR! -->by</label>
               <select name="sort_by">
...
/* Tricky javascript to get the original searchBar absolute cordinates position for the filter searchBar */
        ...
        function GetTextboxWidth(divv){
            var w;
            w = divv.offsetWidth;
            return w;
        }
      
         function GetTextboxCordinates(a,b) {
             
            var txt = document.getElementById(a);
            var p = GetScreenCordinates(txt);
            var query = document.getElementById(b);
            var qw = GetTextboxWidth(txt);
         ...
         document.onreadystatechange = function () {
             ...
             GetTextboxCordinates('query','newSearch');
             var query = document.getElementById('query');
                     
             query.onclick=function(){   /* for search context */
         ...
         window.onresize = function() {
            GetTextboxCordinates('navSearchBar', 'query');
            GetTextboxCordinates('navSearchBarBtn','main-query-submit');
            GetTextboxCordinates('navSearchBarBtn','tlocation');
            GetTextboxCordinates('query','tlocation');
            GetTextboxCordinates('query','newSearch'); 
         };
